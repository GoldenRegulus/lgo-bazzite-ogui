#!/usr/bin/env bash
# Schedule one terminal-backed uninstaller run at the next GNOME login.
set -euo pipefail
...
RUNNER="$SCRIPT_DIR/run-scheduled-uninstall.sh"
AUTOSTART_DIR="$HOME/.config/autostart"
AUTOSTART_FILE="$AUTOSTART_DIR/legion-go-ogui-backend-uninstall.desktop"
INSTALL_AUTOSTART_FILE="$AUTOSTART_DIR/legion-go-ogui-backend-install.desktop"

if [[ ! -f "$RUNNER" ]]; then
  printf '%s\n' 'The bundled scheduled uninstaller runner is missing.' >&2
  exit 4
fi
...
/usr/bin/rm -f -- "$INSTALL_AUTOSTART_FILE"
/usr/bin/printf '%s\n' \
  '[Desktop Entry]' \
  'Type=Application' \
  'Name=Remove Legion Go OGUI backend' \
...
printf '%s\n' 'The uninstaller will open in a terminal at the next GNOME login.'
