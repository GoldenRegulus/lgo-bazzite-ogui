...
finish() {
  ...
  if ! restore_input; then
    printf '%s\n' "hid-driver deactivate: InputPlumber restoration failed" >&2
    rc=1
  fi
  exit "$rc"
}
...
if systemctl is-active --quiet "$unit"; then
  systemctl stop "$unit" || fail "service stop failed; recovery state was kept"
else
  /usr/local/lib/legion-go-ogui/hid/current/unload.sh || \
    fail "driver stop failed; recovery state was kept"
fi
restore_input || fail "InputPlumber restoration failed"
input_active=0
trap - EXIT INT TERM HUP
printf '%s\n' "hid-driver deactivate: stock module is active"
